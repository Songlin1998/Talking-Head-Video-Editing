from load_audface_multiid_lip import load_audface_data_lip
import os
import sys
import numpy as np
# import imageio
...
from run_nerf_helpers_deform_lip import *
import os
from torch.utils.tensorboard import SummaryWriter
from se2e_network.model_s2e_transformer import Speech2Expression_Transformer, AudioNet_ds
from se2e_network.model_s2e_transformer import LipNet
...
def render_dynamic_face_new(H, W, focal, cx, cy, chunk=1024*32, rays=None, bc_rgb=None, aud_para=None,
    ...
    # print('embedded_viewpoints[:,None,None]:',embedded_viewpoints[:,None,None].shape) # torch.Size([4, 1, 1, 33])
    # print('attention_images.shape[:-1] + (attention_embed_ln,):',attention_images.shape[:-1] + (attention_embed_ln,)) # torch.Size([4, 512, 512, 33])
    # bc_viewpoints = torch.broadcast_to(embedded_viewpoints[:,None,None], attention_images.shape[:-1] + (attention_embed_ln,))
    bc_viewpoints = embedded_viewpoints[:,None,None].repeat(1,512,512,1) # 将广播机制替换为repeat操作
    print()
    if use_render_pose:
    ...
    rgb_vp=torch.cat((attention_embed_fn(attention_images),bc_viewpoints),-1)
...
def create_nerf(args):
    ...
    position_warp = Position_warp(319, args.num_reference_images).to(device) #255-14=241
    ...
    nerf_model = Face_Feature_NeRF(D=args.netdepth, W=args.netwidth,
                     input_ch=input_ch, dim_aud=args.dim_exp,
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        LipNet_state = ckpt['network_lipnet_state_dict']
    ...
    return render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, models, LipNet_state, optimizer_Lip_state
...
def config_parser():
    ...
    parser.add_argument("--N_rand", type=int, default=340,
    ...
    parser.add_argument("--dim_aud", type=int, default=64, ##################################修改为表情系数的维度
                        help='dimension of audio features for NeRF')
    parser.add_argument("--dim_exp", type=int, default=56+72, ##################################修改为表情系数的维度
                        help='dimension of deca exp features for NeRF')
    ...
    parser.add_argument("--deca_exp_file", type=str, default='deca_exp.npy',
                        help='deca expression file')
    parser.add_argument("--lip_file", type=str, default='lips.npy',
                        help='lips file')
...
def train():
    ...
    if args.dataset_type == 'audface':
        ...
        images, poses, auds, bc_img, hwfcxy, lip_rects, torso_bcs, _ , deca_exps,mouths,lips= \
            load_audface_data_lip(args.datadir, args.testskip, args.test_file, args.aud_file, need_lip=True, need_torso = args.need_torso, bc_type=args.bc_type, deca_file=args.deca_exp_file, lip_file=args.lip_file)
    ...
    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, models, LipNet_state, optimizer_Lip_state = create_nerf(args)
    ...
    Lip_feature_extractor = LipNet(input_num=51200, output_num=72)
    optimizer_Lip = torch.optim.Adam(
    params=list(Lip_feature_extractor.parameters()), lr=args.lrate, betas=(0.9, 0.999))

    if LipNet_state is not None:
        Lip_feature_extractor.load_state_dict(LipNet_state, strict=False)
    ...
    if args.render_only:
        ...
        # # model
        # model_se2e = Speech2Expression_Transformer(n_acoustic=72+56, n_expression=56).to(device)
        # # audio 处理
        # AudNet = AudioNet_ds(batchsize=8, num_frame=51).to('cuda')
        # checkpoint_path_se2e = '/hd2/yangsonglin/Speech2Expression/save_model/model_log_se2e_finetune/save_models_white/train_s2e_s2s/snapshot.epoch:4000'
                                
        # checkpoint = torch.load(checkpoint_path_se2e, map_location='cpu')
        # model_se2e.load_state_dict(checkpoint['model_state_dict'])
        # AudNet.load_state_dict(checkpoint['Aud_state_dict'])
        # T = 51
        # model_se2e.eval()
        # AudNet.eval()
        ...
        with torch.no_grad():
            ...
            images_refer, poses_refer, auds_refer, bc_img_refer, _ , lip_rects_refer, torso_bcs, _, deca_exps,mouths,lips = \
                load_audface_data_lip(args.datadir, args.testskip, 'transforms_train.json', args.aud_file, need_lip=True, need_torso=True, bc_type=args.bc_type,deca_file=args.deca_exp_file, lip_file=args.lip_file)
            
            auds_refer = torch.Tensor(auds_refer).to(device).float()
            deca_exps = torch.Tensor(deca_exps).to(device).float() # torch.Size([357, 56])
            lips = torch.Tensor(lips).to(device).float() 
            lips_feature = Lip_feature_extractor.forward(lips)# torch.Size([357, 72])
            deca_exps = torch.cat([deca_exps,lips_feature],1) # 直接将连个cat起来
            

            # 直接用原始的素材插入到原位置
            audio_insert = auds_refer[0:10,:]
            deca_exps_gt = deca_exps[0:10,:]
            ...
            # 原视频采样
            # audio_before = auds_refer[start-(T-length)//2:start,:,:].squeeze() 
            # audio_after = auds_refer[start:start+(T-length-(T-length)//2),:,:].squeeze()
            # deca_before = deca_exps[start-(T-length)//2:start,:]
            # deca_after = deca_exps[start:start+(T-length-(T-length)//2),:]

            # 组合
            # deca_insert = deca_exps[start-(T-length)//2:start+(T-length-(T-length)//2),:].mean(axis=0,keepdim=True).repeat(length,1)
            # audio_input = torch.cat((audio_before,audio_insert,audio_after),0) # [51,16,29]
            # deca_input = torch.cat((deca_before,deca_insert,deca_after),0) # [51,56]

            # 提取语音features，并将其处理为 [batchsize, T, 72]
            # audio_feats = audio_input.unsqueeze(0).repeat(8,1,1,1) # reshape成[8,51,16,29]
            # audio_feats = AudNet.forward(audio_feats)
        
            # video_feats = deca_input.unsqueeze(0).repeat(8,1,1) # reshape成[8,51,56] 
            
            # input_audio_expression = torch.cat((audio_feats,video_feats),2)
            # print('input_audio_expression:',input_audio_expression.shape)

            # 预测
            # deca_predict = model_se2e(input_audio_expression.cuda()) # 输出为[8,51,56]
            # deca_predict = deca_predict[0] # 取[51,56]

            # 提取deca系数
            # deca_insert_predict = deca_predict[(T-length)//2:((T-length)//2+length),:]
            # 提取deca系数：相同说话的内容，不同的表情
            # deca_insert_predict = torch.cat([deca_exps[52:60,:56],deca_exps[52:60,56:]],1)
            # deca_insert_predict_novel_expression = torch.cat([deca_exps[82:90,:56],deca_exps[52:60,56:]],1)
            # 提取deca系数：选择特定帧
            # selected_frames = [80,81,82,83,84,85,86,87,88,89,90,91,92,93,94,95,96,97,98,99,100]
            # deca_insert_predict = deca_exps[selected_frames,:]
            # 提取deca系数：不同说话的内容，相同的表情
            deca_insert_predict = deca_exps[88:108,:]

            # print('deca_insert_predict.shape:',deca_insert_predict.shape)
            # print('predict_gt_loss:',F.l1_loss(deca_insert_predict,deca_exps_gt, reduction='mean'))
            ...
            print('poses.shape:',poses_refer.shape)

            deca_predict_save_dir = os.path.join(basedir, expname_finetune, '__dfrf_predict_renderonly_{}_{:06d}'.format(
                'test' if args.render_test else 'path', start))
            os.makedirs(deca_predict_save_dir, exist_ok=True)

            # testsavedir = os.path.join(basedir, expname_finetune, 'renderonly_novel_expression_{}_{:06d}'.format(
            #     'test' if args.render_test else 'path', start))
            # os.makedirs(testsavedir, exist_ok=True)

            # inference：相邻帧采样
            perm = [start-1,start,end,end+1]
            ...
            torso_bcs_insert = []
            for j in range((length-alter_length)//2):
                torso_bcs_insert.append(torso_bcs[start])
            for j in range(start,end):
                torso_bcs_insert.append(torso_bcs[j])
            for j in range((length-alter_length)-(length-alter_length)//2):
                torso_bcs_insert.append(torso_bcs[end])
            poses_refer = poses_refer.cuda()
            poses_insert = torch.cat([poses_refer[start].repeat((length-alter_length)//2,1,1),
                                    poses_refer[start:end],
                                    poses_refer[end].repeat(((length-alter_length)-(length-alter_length)//2),1,1)],0)

            print(torso_bcs_insert)
            print('poses_insert.shape:',poses_insert.shape)
            
            # inference：特定帧采样
            # perm = selected_frames
            # perm = perm[0:args.num_reference_images]
            # attention_images = images_refer[perm].to(device)
            # print('attention_images.shape:',attention_images.shape)
            # attention_poses = poses_refer[perm, :3, :4].to(device)
            ...
            #     torso_bcs_insert.append(torso_bcs[j])
            # poses_refer = poses_refer.cuda()
            # poses_insert = poses_refer[selected_frames,:,:]
            # print('poses_insert.shape:',poses_insert.shape)
            
            # with torch.no_grad():
            #     rgbs, disp, last_weight = render_path(args, torso_bcs_insert, poses_insert, deca_insert_predict, bc_img, hwfcxy, attention_poses,attention_images,
            #                 intrinsic, args.chunk, render_kwargs_test, gt_imgs=None, savedir=deca_predict_save_dir, lip_rect=lip_rects_refer[perm])
                # rgbs, disp, last_weight = render_path(args, torso_bcs, poses[:3,], deca_exps[:3,], bc_img, hwfcxy, attention_poses,attention_images,
                #             intrinsic, args.chunk, render_kwargs_test, gt_imgs=None, savedir=deca_predict_save_dir, lip_rect=lip_rects_refer[perm])
            # np.save(os.path.join(deca_predict_save_dir, 'last_weight.npy'), last_weight)
            # imageio.mimwrite(os.path.join(
            #     deca_predict_save_dir, 'length.mp4'), to8b(rgbs), fps=25, quality=8)

            # with torch.no_grad():
            #     rgbs, disp, last_weight = render_path(args, torso_bcs_insert, poses_insert, deca_insert_predict_novel_expression, bc_img, hwfcxy, attention_poses,attention_images,
            #                 intrinsic, args.chunk, render_kwargs_test, gt_imgs=None, savedir=testsavedir, lip_rect=lip_rects_refer[perm])

            # 生成视频：包含predict（长度：T）

            # torso_bcs[290:325] = torso_bcs[230:265]
            # poses_refer[290:325] = poses_refer[230:265]
            # deca_exps[290:325] = deca_exps[230:265]

            with torch.no_grad():
                rgbs, disp, last_weight = render_path(args, torso_bcs, poses_refer, deca_exps, bc_img, hwfcxy, attention_poses,attention_images,
                            intrinsic, args.chunk, render_kwargs_test, gt_imgs=None, savedir=deca_predict_save_dir, lip_rect=lip_rects_refer[perm])

            np.save(os.path.join(deca_predict_save_dir, 'last_weight.npy'), last_weight)
            print('Done rendering', deca_predict_save_dir)
            imageio.mimwrite(os.path.join(
                deca_predict_save_dir, 'video.mp4'), to8b(rgbs), fps=25, quality=8)
            return
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            deca_exps_ = torch.Tensor(deca_exps[str(select_id)]).to(device).float()
            ...
            # else:
            #     print('aud (before process):',aud.shape) # torch.Size([16, 29])
            #     aud = AudNet(aud.unsqueeze(0))
            #     print('deca_exp:',deca_exp.shape)
            #     print('aud:',aud.shape) # torch.Size([64])
        ...
        #                                                     attention_poses = attention_poses, intrinsic = intrinsic, render_pose=pose,lip_rect = lip_rect, **render_kwargs_train)
        rgb, disp, acc, _, extras, loss_translation = render_dynamic_face_new(H, W, focal, cx, cy, chunk=args.chunk, rays=batch_rays,
                                                            aud_para=deca_exp, bc_rgb=bc_rgb,
                                                            verbose=i < 10, retraw=True, attention_images = attention_images,
                                                            attention_poses = attention_poses, intrinsic = intrinsic, render_pose=pose,lip_rect = lip_rect, **render_kwargs_train)
        ...
        if i % args.i_weights == 0:
            ...
            torch.save({
                ...
                'network_lipnet_state_dict': Lip_feature_extractor.state_dict(),
