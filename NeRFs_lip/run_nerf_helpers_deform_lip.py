...
def make_indices(pts, attention_poses, intrinsic, H, W):
    # print('torch.tensor([1.0]):',torch.tensor([1.0]).shape) # torch.Size([1])
    # print('pts.shape[:-1] + (1,):',pts.shape[:-1] + (1,)) # torch.Size([54400，1])
    # hom_points = torch.cat((pts, torch.broadcast_to(torch.tensor([1.0]), pts.shape[:-1] + (1,))), -1)
    hom_points = torch.cat((pts, torch.ones(pts.shape[:-1] + (1,))), -1)
    extrinsic = invert(attention_poses)[:, :3]
    focal = intrinsic[0, 0]

    # print('hom_points[None, ...]:',hom_points[None, ...].shape) # torch.Size([1, 54400, 4])
    # print('(extrinsic.shape[0], hom_points.shape[0], hom_points.shape[1]):',(extrinsic.shape[0], hom_points.shape[0], hom_points.shape[1])) # (4, 54400, 4)
    ...
    pt_camera = hom_points[None, ...].repeat(4,1,1) @ extrinsic.permute(0, 2, 1)
...
class nerf_attention_model(nn.Module):
    ...
    def forward(self, inputs, training=None):
        ...
        # print('embedded_pts[None]:',embedded_pts[None].shape) # torch.Size([1, 54400, 63])
        # print('(local.shape[0], local.shape[1], embedded_pts.shape[-1]):',(local.shape[0], local.shape[1], embedded_pts.shape[-1])) # (4, 54400, 63)
        # embedded_pts = torch.broadcast_to(embedded_pts[None], (local.shape[0], local.shape[1], embedded_pts.shape[-1]) )
        embedded_pts = embedded_pts[None].repeat(4,1,1)
...
class Position_warp(nn.Module):
    ...
    def forward(self, query_pts, aud, reference_feature):
        ...
        # print('query_pts.shape[0],64:',query_pts.shape[0],64) # 54400 64
        input = torch.cat((query_pts, aud.repeat(query_pts.shape[0],1), reference_feature),1)
