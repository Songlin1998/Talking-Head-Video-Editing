...
from se2e_network import utils_fast
...
class SinusoidalPositionalEmbedding(nn.Module):
    ...
    def forward(self, input, incremental_state=None, timestep=None, **kwargs):
        ...
        positions = utils_fast.make_positions(input, self.padding_idx)
...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        attn_weights_float = utils_fast.softmax(attn_weights, dim=-1)
    ...
    def _get_input_buffer(self, incremental_state):
        return utils_fast.get_incremental_state(
            self,
            incremental_state,
            'attn_state',
        ) or {}
    ...
    def _set_input_buffer(self, incremental_state, buffer):
        utils_fast.set_incremental_state(
            self,
            incremental_state,
            'attn_state',
            buffer,
...
class EncLocalSALayer(nn.Module):
    ...
    def forward(self, x, encoder_padding_mask=None, **kwargs):
        ...
        states = []
        T = x.shape[0]
        all_neg_inf = utils_fast.fill_with_neg_inf2(x.new(T, T))
...
class LinearizedConvolution(ConvTBC):
    ...
    def _get_input_buffer(self, incremental_state):
        return utils_fast.get_incremental_state(self, incremental_state, 'input_buffer')

    def _set_input_buffer(self, incremental_state, new_buffer):
        return utils_fast.set_incremental_state(self, incremental_state, 'input_buffer', new_buffer)
...
class DecConvLayer(nn.Module):
    ...
    def forward(self, x, encoder_out=None, encoder_padding_mask=None, incremental_state=None, **kwargs):
        ...
        x, attn = self.attention(
            ...
            static_kv=True,
            enc_dec_attn_constraint_mask=utils_fast.get_incremental_state(self, incremental_state,
...
class DecSALayer(nn.Module):
    ...
    def set_buffer(self, name, tensor, incremental_state):
        return utils_fast.set_incremental_state(self, incremental_state, name, tensor)
...
class DecLSTMLayer(nn.Module):
    ...
    def forward(self, x, encoder_out=None, encoder_padding_mask=None, incremental_state=None, **kwargs):
        ...
        if incremental_state is not None:
            x = x[-1:, :, :]
        cached_state = utils_fast.get_incremental_state(self, incremental_state, 'cached_state')
