...
def create_nerf(args):
    ...
    position_warp = Position_warp(319, args.num_reference_images).to(device) #255-8=247
    ...
    return render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, models
...
def config_parser():
    ...
    parser.add_argument("--dim_exp", type=int, default=56+72, ##################################修改为表情系数的维度+lip的维度
...
def train():
    ...
    if args.dataset_type == 'audface':
        if args.with_test == 1:
            ...
            images, poses, auds, bc_img, hwfcxy, sample_rects, i_split, id_num, lip_rects, torso_bcs, deca_exps,mouths,lips = load_audface_data_lip(
                args.datadir, args.testskip, train_length = args.train_length, need_lip=True, need_torso = args.need_torso, bc_type=args.bc_type, deca_file=args.deca_exp_file,lip_file=args.lip_file)
    ...
    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, models = create_nerf(args)
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            lips_ = torch.Tensor(lips[str(select_id)]).to(device).float()

            i_train, i_val = i_split[str(select_id)]
            img_i = np.random.choice(i_train)
            image_path = images[str(select_id)][img_i]
            mouth_path = mouths[str(select_id)][img_i]
            ...
            target_mouth = torch.as_tensor(imageio.imread(mouth_path)).to(device).float()/255.0
            pose = poses_[img_i, :3, :4]
            rect = sample_rects[str(select_id)][img_i]
            aud = auds_[img_i]
            deca_exp = deca_exps_[img_i]
            lip = lips_[img_i]
            lip = Lip_feature_extractor.forward(lip)
            ...
            if N_rand is not None:
                ...
                target_s = target[select_coords[:, 0],
                                  select_coords[:, 1]]  # (N_rand, 3)
                target_m = target_mouth[select_coords[:, 0],
                                  select_coords[:, 1]]
        ...
        #                                                     attention_poses = attention_poses, intrinsic = intrinsic, render_pose=pose,lip_rect = lip_rect, **render_kwargs_train)
        rgb, disp, acc, _, extras, loss_translation = render_dynamic_face_new(H, W, focal, cx, cy, chunk=args.chunk, rays=batch_rays,
                                                            aud_para=torch.cat([deca_exp,lip],0), bc_rgb=bc_rgb,
                                                            verbose=i < 10, retraw=True, attention_images = attention_images,
                                                            attention_poses = attention_poses, intrinsic = intrinsic, render_pose=pose,lip_rect = lip_rect, **render_kwargs_train)
        ...
        # mouth_loss = img2mse(torch.mul(rgb,(1-target_m)),torch.mul(target_s,(1-target_m)))
        trans = extras['raw'][..., -1]
        ...
        if i % args.i_weights == 0:
            path = os.path.join(basedir, expname_finetune+'_lip', '{:06d}_head.tar'.format(i))
            os.makedirs(os.path.join(basedir, expname_finetune+'_lip'), exist_ok=True)
            torch.save({
